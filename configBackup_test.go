...
func TestValidConfigWithNotifications(t *testing.T) {
	quietFlag = true
	defer func() {
		quietFlag = false
	}()
	...
	// Read the expected configuration file under test
	configFile = newConfigurationFile()
	configFile.setConfig("arrayWithNotifications")
	globalStorageDirectory = "test/assets/backupConfigs/"
	channels = notificationChannels{
		OnStart:   []string{"http"},
		OnSkip:    []string{"http"},
		OnSuccess: []string{"http"},
		OnFailure: []string{"http"},
	}
	if err := configFile.loadConfig(false, false); err != nil {
		t.Error(err)
	}
	...
	if len(onStartNotifiers) != 1 {
		t.Error("there should have been 1 onStart notifier")
	}
	if len(onSkipNotifiers) != 0 {
		t.Error("there should have been 0 onSkip notifier")
	}
	if len(onSuccessNotifiers) != 1 {
		t.Error("there should have been 1 onSuccess notifier")
	}
	if len(onFailureNotifiers) != 1 {
		t.Error("there should have been 1 onFailure notifier")
	}
}
